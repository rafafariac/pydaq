...
class Send_data(Base):
    ...
    def send_data_nidaq_gui(self):
        ...
        bottom_line = [[sg.Button("SEND DATA", key="-Start-", auto_size_button=True)]]
        ...
        while True:
            ...
            if event == "-Start-":
                ...
                if (max(self.data) > float(self.ao_max)) or (
                    min(self.data) < float(self.ao_min)
                ):
                    self._range_error()
                    self.error_max = True
                else:
