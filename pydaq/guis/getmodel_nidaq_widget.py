...
class GetModel_Nidaq_Widget(QWidget, Ui_Arduino_GetModel_W):
    def __init__(self, *args):
        ...
        # Connecting signals
        self.terminal_config_in.addItems(["Diff", "RSE", "NRSE"])
        self.choose_device_in.addItems(self.device_type)
        ...
        self.choose_channel_in.addItems(chan)
        self.path_line_edit.setText(
            os.path.join(os.path.join(os.path.expanduser("~")), "Desktop")
        )
        defchan_index = self.choose_channel_in.findText(defchan)
        self.choose_device_in.currentIndexChanged.connect(self.update_channels)

        if defchan_index == -1:
            pass
        else:
            self.choose_channel_in.setCurrentIndex(defchan_index)
    ...
    def start_func_get_model(self):  # Start getting model
        try:
            ...
            g.device = self.choose_device_in.currentText().split("/")[0]
            g.channel = self.choose_channel_in.currentText().split("/")[1]
            g.terminal = g.term_map[self.terminal_config_in.currentText()]
