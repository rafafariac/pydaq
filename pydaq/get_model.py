...
class GetModel(Base):
    ...
    def get_model_nidaq(self):
        ...
        if self.plot:  # If plot, start updatable plot
            self.title = f"PYDAQ - Geting Data (NIDAQ). {self.device}, {self.ai_channel}, {self.ao_channel}"
            self._start_updatable_plot()

        sent_data = [b"1" if i == 1 else b"0" for i in sinal]

        for k in range(self.cycles):

            task.write(sent_data)
            ...
            if self.plot:
                ...
                self._update_plot(
                    [self.time_var, self.time_var],
                    [self.sinal[0 : k + 1] * 5, self.out_read[0 : k + 1]],
                    2,
                )
            print(f"Iteration: {k} of {self.cycles-1}")
    ...
    def show_results(self, results_dataframe):
        # não ta certo quando tem uma parte constante (regressor 1)
        # rever as condicionais de sinal, tá ruim
        model_string = "y_k = "
        r = results_dataframe
        line_control = 0
        string_list = []
        ...
        if self.acquired_model.basis_function.degree == 1:
            numerator_string = ""
            denominator_string = "1"
            for i in range(self.acquired_model.n_terms):
                if np.max(self.acquired_model.final_model[i]) < i:
            ...
            # tem alguma coisa dando errado aqui acima
            print(numerator_string)
            print(denominator_string)
            latex_eq += f"\\frac{{{numerator_string}}}{{{denominator_string}}}"
            print(latex_eq)

            # plt.text(0.5, 0.5, f"${latex_eq}$", fontsize=20, ha="center", va="center")
